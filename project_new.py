...
connection = mysql.connector.connect(
    host="127.0.0.1",
    user="root",
    password="Krishnaveni@1",
    database="youtube_project"
)
cur = connection.cursor()

# Function to extract channel data :-
def get_channel_data(res_cha ,res_playlist ):          
        
    title = res_cha['items'][0]['snippet']['localized']['title']
    Channel_Id = res_cha['items'][0]['id']
    Subscription_Count = res_cha['items'][0]['statistics']['subscriberCount']
    Channel_Views = res_cha['items'][0]['statistics']['viewCount']
    Channel_Description = res_cha['items'][0]['snippet']['description']
    ...
    channel_dict[f"{title}"] = channel_data
    return channel_dict

# Function to extract comments data :-    
...
# Function to extract Videos data :-
def get_videos_data(youtube,response_video_details):    
    res_vid = response_video_details
    items = res_vid.get('items',[])
    ...
    for index, item in enumerate(items, start=1):
        ...
        comments = comment_data(youtube, video_id)
        video_data = {
            'Video_Id': video_id,
        ...
        videos_data[f"video_id_{index}"] = video_data

    return videos_data


### Inserting the datas to My_SQL :-

def sql_data(cha_det,res_playlist):
    Playlist_Id = res_playlist['items'][0]['id']
    data = cha_det
    ...
    for key, value in data.items():
        ...
        if 'Video_Id' in value:
            ...
            video_data= (value.get('Video_Id', ''),Playlist_Id, 
        ...
        if 'Comments' in value: # here check comments 
...
# Main function to merge all the above functions :-

def main(channel_id):
    api_key = "AIzaSyDB0tmLByqbOI0yFHWeVRzFHmokg1UD0FE"
    id = channel_id
    youtube = googleapiclient.discovery.build("youtube", "v3", developerKey=api_key)    

    res_cha = youtube.channels().list(
    ...
    upload_playlist_id = res_cha['items'][0]['contentDetails']['relatedPlaylists']['uploads']
    ...
    cha_det = get_channel_data(res_cha ,res_playlist )
    vid_det = get_videos_data(youtube,response_video_details)
    cha_det.update(vid_det)
    pprint.pprint(cha_det)
    
    sql_data(cha_det,res_playlist)
    

st.title('YOUTUBE DATA HARVESTING') 
st.text("Enter YouTube channel_id below:")
st.text("Hint: go to channel's home page >> right  click >> view page resource >>Find channel_id")
channel_id = st.text_input("Enter Channel ID:", "") 
if st.button("Extract Data to SQL"):
    main(channel_id)
